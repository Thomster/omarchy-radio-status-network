...
Panel {
  ...
  // A wifi radio deliberately switched off (not merely unassociated) is a
  // distinct state from "disconnected" -- there is a station, it just isn't
  // trying. No wifi hardware at all falls through to plain "disconnected"
  // since there is nothing to describe as disabled.
  readonly property bool wifiRadioOff: !!wifiDevice && !Networking.wifiEnabled
  readonly property string kind: {
    if (wiredDevice && wiredDevice.connected) return "ethernet"
    if (connectedWifiNetwork) return "wifi"
    if (wifiRadioOff) return "disabled"
    return "disconnected"
  }
        ...
        Column {
          ...
          Text {
            ...
            readonly property string title: {
              ...
              if (root.kind === "disabled") return "Disabled"
